...
def flagData(data, nwindow=100001, signific=5, filtertype=0, badneighbors=4):
    return np.copy(flagDataV(np.copy(data.T), nwindow, signific, filtertype, badneighbors).T)

@nb.guvectorize([(nb.float64[:], nb.int64, nb.int64, nb.int64, nb.int64, nb.boolean[:])],'(n),(),(),(),()->(n)', target='parallel')
