...
def load_pulsar_data(
    pulsar_name, pulsarcat_file=Path(__file__).parent / "small-data-files/pulsarcat.csv"
):
    # read the data of the pulsar database
    pulsardata = np.genfromtxt(
        pulsarcat_file, delimiter=",", dtype=None, names=True, missing_values="*"
    )
    # Find the index of the correct pulsar
    matches = np.where(
        (pulsardata["NAME"].astype(str) == pulsar_name)
        | (pulsardata["PSRJ"].astype(str) == pulsar_name)
    )
...
class Pulsar:
    ...
    def __init__(
        self, pulsardata=None, pulsarname=None, tobs=Time.now(), chisqperiod=None
    ):
