...
from tqdm import (
    tqdm,
)  # A nice progress bar, can be installed using, even on astro computers with: pip install --user tqdm
...
@njit
def timeFolding(
    data_array, nbins, period, flagged=None, corrected_times=None, progressbar=True
):
    if flagged is None:
    ...
    normalisedfoldedarray = protonormalisedfoldedarray / protonormalisedfoldedarray.sum(
        axis=0
    )
